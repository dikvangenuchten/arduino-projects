...
#[test]
fn boot_shows_the_speed_view_with_all_relays_on() {
...
#[test]
fn pressing_an_external_relay_key_turns_that_relay_off_once() {
    let (mut ctrl, state) = new_controller();
...
#[test]
fn release_then_repress_toggles_the_relay_on_again() {
    let (mut ctrl, state) = new_controller();
    state.borrow_mut().inputs[0] = true;
    tick_n(&mut ctrl, DEBOUNCE_SAMPLES);
    assert!(!state.borrow().relay_state[0]);
    ...
    assert!(state.borrow().relay_state[0]);
    assert_eq!(state.borrow().relay_on_calls[0], 2, "one On call from boot, one from the repress");
}

#[test]
fn global_action_turns_all_relays_off_since_boot_starts_all_on() {
    let (mut ctrl, state) = new_controller();
    state.borrow_mut().inputs[5] = true; // I5 = Global, unshifted -> GlobalPower.
    tick_n(&mut ctrl, DEBOUNCE_SAMPLES);

    let state = state.borrow();
    assert_eq!(state.relay_off_calls, [1; RELAY_COUNT]);
    assert_eq!(state.relay_state, [false; RELAY_COUNT]);
}

#[test]
fn global_action_turns_all_relays_back_on_after_turning_them_off() {
    let (mut ctrl, state) = new_controller();
    state.borrow_mut().inputs[5] = true; // First press: all on -> all off.
    tick_n(&mut ctrl, DEBOUNCE_SAMPLES);

    state.borrow_mut().inputs[5] = false;
    tick_n(&mut ctrl, DEBOUNCE_SAMPLES);

    state.borrow_mut().inputs[5] = true; // Second press: all off -> all on.
    tick_n(&mut ctrl, DEBOUNCE_SAMPLES);
